...
public class InvalidOrderStateForPaymentException : BaseException
{
    ...
    private const string DefaultMessage =
        "The order must be in the 'Pending' status to be marked as paid";

    private static readonly ErrorCode _defaultErrorCode = ErrorCode.InvalidOperation;

    internal InvalidOrderStateForPaymentException()
        : base(DefaultMessage, DefaultTitle, _defaultErrorCode)
    {
    }

    internal InvalidOrderStateForPaymentException(string message)
